...
- [ ] Project files — Standard files not yet documented (.gitignore, .gitattributes, rustfmt.toml, clippy.toml, deny.toml, .editorconfig, codecov.yml, CHANGELOG.md, CODEOWNERS, issue/PR templates). Dependency updates are configured through `renovate.json5`; see [CI > Dependency Updates](ci.md#dependency-updates).
...
- [x] OAuth 2.0 authentication — ~~Multi-user apps, team access~~ Implemented for standalone CLI, MCP tool, OpenCode plugin, and experimental self-hosted HTTP; local token file storage uses direct OAuth by default. See [Authentication](authentication.md)
...
- [x] Standalone OAuth flow — ~~Built-in browser/callback flow in the MCP binary itself (currently only via OpenCode plugin)~~ Implemented: `onshape-mcp auth login` CLI subcommand and `onshape_auth_login` MCP tool. Direct mode is the default; proxy mode requires an explicit URL for a [self-hosted OAuth proxy](oauth-proxy.md). See [Authentication](authentication.md#standalone-oauth-flow)
- [ ] MCP login secret arguments — Redesign of direct `client_secret` tool input is deferred to [#548](https://github.com/altendky/onshape-mcp/issues/548).
- [ ] Streamable HTTP client compatibility — ChatGPT currently fails; investigation is tracked in [#546](https://github.com/altendky/onshape-mcp/issues/546). No public endpoint is offered.
