...
- [x] Standalone OAuth flow — ~~Built-in browser/callback flow in the MCP binary itself (currently only via OpenCode plugin)~~ Implemented: `onshape-mcp auth login` CLI subcommand and `onshape_mcp_auth_login` MCP tool. Supports proxy mode (default, delegates token exchange to the [OAuth proxy](oauth-proxy.md)) and direct mode (exchanges tokens directly with Onshape). See [Authentication](authentication.md#standalone-oauth-flow)
